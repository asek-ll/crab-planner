    ...
    public static class JsonHelper implements JsonSerializer<PlannerIngredientStack>, JsonDeserializer<PlannerIngredientStack> {
        ...
        private final LoadingCache<Class<?>, Map<String, Object>> uidCacheByType = CacheBuilder.newBuilder()
                .build(new CacheLoader<Class<?>, Map<String, Object>>() {
                    @Override
                    public Map<String, Object> load(Class<?> ingredientClass) throws Exception {
                        IIngredientManager ingredientManager = CrabJeiPlugin.getJeiRuntime().getIngredientManager();
                        IIngredientType<Object> ingredientType = ingredientManager.getIngredientType(ingredientClass);
                        final Collection<?> allIngredients = ingredientManager.getAllIngredients(ingredientType);
                        final IIngredientHelper<Object> ingredientHelper = ingredientManager.getIngredientHelper(ingredientType);
        ...
        public JsonElement serialize(PlannerIngredientStack src, Type typeOfSrc, JsonSerializationContext context) {
            ...
            final String uniqueId = CrabJeiPlugin.getJeiRuntime().getIngredientManager()
                    .getIngredientHelper(ingredient.getRawStack()).getUniqueId(ingredient.getRawStack());
